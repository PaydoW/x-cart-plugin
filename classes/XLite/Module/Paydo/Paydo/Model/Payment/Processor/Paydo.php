...
namespace XLite\Module\Paydo\Paydo\Model\Payment\Processor;

/**
 * Class Paydo
 *
 * @package XLite\Module\Paydo\Paydo\Model\Payment\Processor
 */
class Paydo extends \XLite\Model\Payment\Base\WebBased
{
    protected $log_name = 'paydo_log';
    ...
    protected function getFormURL()
    {
        if (!isset($this->checkoutURL)){
            $paydoSettings = $this->getPaydoSettings();
            $invoiceId = $this->createInvoice($paydoSettings);
            $this->checkoutUrl = 'https://paydo.com/'.'en'.'/payment/invoice-preprocessing/'.$invoiceId;
        }
    ...
    {
        return 'modules/Paydo/Paydo/config.twig';
    }
     ...
     * @return array
     */
    protected function getPaydoSettings()
    {
        return array(
    ...
    protected function validateCallback($txid, $status, \XLite\Model\Payment\Transaction $transaction)
    {
        ...
        $url = 'https://paydo.com/v1/transactions/'.$txid;
        $ch = curl_init($url);
